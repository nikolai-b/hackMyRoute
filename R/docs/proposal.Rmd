...
When deciding between a number of competing cycle paths we aim to build
a tool to visualise different metrics of the areas involved.
One path may run through an area with a current low cycling rate that
is surrounded by areas of a high cycle rate, one may have a medium cycle rate
but have large propensity to cycle rate with the connecting location.  Having all
of these datasets available in a simple online tool could help decide which
paths could have the most impact.  This would allow tapping into a currently existing
...
metrics that a planner would like to have access to.  The tool would also have the
ability to create their own custom metric based on the available data.
We propose to create this map for Leeds.
